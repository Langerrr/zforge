...
Every file is created from the plugin templates at `../../../templates/`, resolved relative to this `SKILL.md`; phase files come from `../../../templates/05_progress/05_XX_phase_template.md`. Their `## Evidence Required` rows inherit the matrix, and each row's command leaves its output under `/tmp/zforge/artifacts/{feature}/` — outside every repository — so declare nothing in the matrix that cannot leave a file behind.

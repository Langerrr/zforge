...
**A shortfall is settled by whether it matters.** Where achieved falls below required, a material gap opens a standing flag with a stated *closes when* and the feature is not complete while any flag is open; an immaterial one is accepted with the reasoning recorded. Two shortfalls are never immaterial: a command that selected nothing and exited 0, and a user-reachable surface nothing reached. The achieved class is recorded as reached either way. On a long chain the verification is delegated to `zforge:acceptance-agent`, on a model the user chose at pre-flight, and it returns a per-row verdict and a recommendation — it needs a phase file and a shell rather than the run's accumulated context. That report is the `## Acceptance` section, pasted as returned with one line of adjudication; a phase reopened for a fix is re-accepted for the rows the fix touched.

**Decisions do not block, and only the load-bearing ones surface.** Agents decide, record, mark each decision's reach, and keep moving. At acceptance the planner promotes to `decision_review.md` the decisions marked `outward` — reaching beyond the feature's implementation — and the rest stay in their phase file. The ledger is a queue you drain, not an index of everything decided. Blocking is reserved for five named pause triggers.
...
**A reported phase is not an accepted phase.** `REPORTED` is its own state: the work is on disk and the evidence has not been verified. The agent writes it as its last act before reporting, so a planner that never receives the report finds a phase that says on disk exactly what is true of it. `COMPLETED` is written by acceptance and by nothing else.
